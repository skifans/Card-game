...
def txt_button(msg,x,y,w,h,inactive_colour,active_colour,text_colour,name_of_function_to_call_when_clicked, argument1, argument2):
    ...
    if x+w > mouse[0] > x and y+h > mouse[1] > y: #check if mouse is on button
        ...
        if click[0] == 1: #check click (above if checks mouse is on button)
            if argument1 == "" and argument2 == "":
                ...
                name_of_function_to_call_when_clicked(argument1) #do this when clicked (veriable needs not to have brackets)brackets)
            else:
                name_of_function_to_call_when_clicked(argument1,argument2)
    else:
...
def img_button(x,y,w,h,name_of_function_to_call_when_clicked,filename,argument1, argument2):
    #need pygame.flip/update outside of function
    click = pygame.mouse.get_pressed() #get mouse state (clicked/not clicked)
    mouse = pygame.mouse.get_pos() #get mouse coords
...
def keyboard_example():
    global InputOn
    InputOn=2 #letters & numbers (1 for letters only)
    b=keyboard(2) #2=< charectors needed
    print(b)
        ...
        for event in pygame.event.get():
            ...
            if event.type==KEYDOWN:
                ...
                if len(string)>minlength-1: #make sure that at least one letter has been entered (0 letters was also causing a crash)
                    txt_button('Finnished',213.5,260,402,30,WHITE,GREY,BLACK,closeinput, string) #calls close input function
                    txt_button('Clear',213.5,290,201,30,WHITE,GREY,BLACK,clear, "")
                    txt_button('Backspace',404.5,290,201,30,WHITE,GREY,BLACK,backspace, "")
                else:
...
def closeinput(string):
    ...
    keyboard(0) #calls back function (without buttons) so that text can be returned via the original function (user dosnt need to enter anything so 0 min length is fine)
...
def deckBuilderScreen():
    ...
    while deckBuilder == True:
        for event in pygame.event.get():
            ...
            txt_button("Single Player Deck", 200, 600, 150, 75, GREEN, DARKGREEN, BLACK, classPicker, "single", "")
            txt_button("Multi Player Deck", 400, 600, 150, 75, GREEN, DARKGREEN, BLACK, classPicker, "multi", "")
            pygame.display.flip()

def classPicker(mode):
    classPicker = True
    while classPicker == True:
        for event in pygame.event.get():
            wn.fill(DARKPINK)
            txt_button("Return", 200, 200, 75, 75, GREEN, DARKGREEN, BLACK, menuFunction, "", "")
            txt_button("Exit", 600, 200, 75, 75, GREEN, DARKGREEN, BLACK, terminate, "", "")
            ...
            txt_button("Palladin", 200, 600, 150, 100, GREEN, DARKGREEN, BLACK, cardPicker, mode, "palladin" )
            txt_button("Mage", 400, 600, 150, 100, GREEN, DARKGREEN, BLACK, cardPicker, mode, "mage")
            txt_button("Smith", 600, 600, 150, 100, GREEN, DARKGREEN, BLACK, cardPicker, mode, "smith")
        pygame.display.flip()
...
def displayCard(card,cardKey, deck):
    ...
    img_button(cardOneTemplate[0], cardOneTemplate[1], cardOneTemplate[2], cardOneTemplate[3], cardSelect, card, card, deck)
        #Description Layout
...
def cardPicker(mode, playerClass):
    cardPicker = True
    while cardPicker == True:
        for event in pygame.event.get():
            ...
            if mode == "single":
                if playerClass == "palladin":
                    displayCard("1.jpg","0001", mainDeckSingle)
                elif playerClass == "mage":
                    displayCard("2.jpg","0002", mainDeckSingle)
                elif playerClass == "smith":
                    displayCard("3.jpg", "0002", mainDeckSingle)
            elif mode == "multi":
                if playerClass == "palladin":
                    displayCard("1.jpg","0001", mainDeckMulti)
                elif playerClass == "mage":
                    displayCard("2.jpg", "0002", mainDeckMulti)
                elif playerClass == "smith":
                    displayCard("3.jpg", "0003", mainDeckMulti)
        pygame.display.flip()
