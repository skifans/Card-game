...
def message_display_left(text, x, y, font_size, colour):
    largeText = pygame.font.Font('freesansbold.ttf',font_size) #load font
    TextSurf, TextRect = text_objects(text, largeText, colour) #render text
    TextRect.topleft = ((x),(y)) #place text
    #screen=pygame.display.set_mode((0,0)) uncomenting this lets fixes the screen not defined bug - but also causes problems displaying text if let uncommented.
    screen.blit(TextSurf, TextRect) #send to screen, needs to be updated/fliped to be worked
...
def showcardsheld(hand):
    print(hand)
    for i in range(len(hand)):
        displaycardsheld(i,hand)

def displaycardsheld(id, hand):
    print("displaying card: ",hand[id])
    if hand[id][0]==0:
        #type (0 = weapon, 1 = gloves, 2 = legs, 3= face, 4 = chest), attack/defense, durability, mana cost (?) name, effect.

        #weapon
        image = pygame.image.load(os.path.join("cards","weapon base.jpg"))
        screen.blit(image, ((id*120)+100,400)) #the location of the image
        message_display(""+str(hand[id][1]),(id*120)+108,504,12,WHITE) #attack
        message_display(""+str(hand[id][3]),(id*120)+155,581,12,BLACK) #mana cost (?) - where is this saved?
        message_display(""+str(hand[id][2]),(id*120)+208,504,12,BLACK) #durability
        message_display(""+str(hand[id][4]),(id*120)+155,540,11,BLACK) #name
        try:
            image=pygame.image.load(os.path.join("card forgrounds",str(hand[id][4])+".PNG"))
            #image=pygame.image.load(os.path.join("card forgrounds","Placeholder.png"))
            screen.blit(image, (id*120+110,410))
        except:
            image=pygame.image.load(os.path.join("card forgrounds","Placeholder.png"))
            screen.blit(image, (id*120+110,410))
    ...
    if hand[id][0]>0:
        ...
        screen.blit(image, ((id*120)+100,400)) #the location of the image
        message_display(""+str(hand[id][1]),(id*120)+108,504,12,WHITE) #defence
        message_display(""+str(hand[id][3]),(id*120)+155,581,12,BLACK) #mana cost (?)
        message_display(""+str(hand[id][2]),(id*120)+208,504,12,BLACK) #durability
        message_display(""+str(hand[id][4]),(id*120)+155,540,11,BLACK) #name
        try:
            image=pygame.image.load(os.path.join("card forgrounds",str(hand[id][4])+".PNG"))
            #image=pygame.image.load(os.path.join("card forgrounds","Placeholder.png"))
            screen.blit(image, ((id*120)+115,410))
        except:
            print("Card forground for ",str([hand[id][4]]).lower(), "not found.")
            print(sys.exc_info())
            image=pygame.image.load(os.path.join("card forgrounds","Placeholder.png"))
            screen.blit(image, ((id*120)+115,410))
...
def muilti(blank): #muiltiplayer function
    ...
    def set_player_1(blank):
        global turn_1, health_1, amour_mod_1, weapon_1, name_1, deck_1, hand_1
        turn_1 = 0
        health_1 = 20
    ...
    def set_player_2(blank):
        global turn_2, health_2, amour_mod_2, weapon_2, name_2, deck_2, hand_2
        turn_2 = 0
        health_2 = 20
                     #[head  ,chest ,arms  ,legs  ]
        amour_mod_2 = [[0,''],[0,''],[0,''],[0,'']]
    ...
    def play_multi(blank):
        set_player_1('')
        set_player_2('')
        def player_1_stats(health_1, amour_mod_1, weapon_1):
            text = 'Health: ' + str(health_1)
            message_display_left(text,50,25,15,BLACK)
            text = 'Armour: '+str(amour_mod_1[0][0]+amour_mod_1[1][0]+amour_mod_1[2][0]+amour_mod_1[3][0])
            message_display_left(text,50,40,15,BLACK)
            text = '    head: '+str(amour_mod_1[0][0])
            message_display_left(text,50,55,15,BLACK)
            text = '    chest: '+str(amour_mod_1[1][0])
            message_display_left(text,50,70,15,BLACK)
            text = '    arms: '+str(amour_mod_1[2][0])
            message_display_left(text,50,85,15,BLACK)
            text = '    legs: '+str(amour_mod_1[3][0])
            message_display_left(text,50,100,15,BLACK)
            text = 'Weapons: '
            message_display_left(text,50,115,15,BLACK)
            text = '    name: '+str(weapon_1[0])
            message_display_left(text,50,130,15,BLACK)
            text = '    attack: '+str(weapon_1[1])
            message_display_left(text,50,145,15,BLACK)
            pygame.display.flip()

        def player_2_stats(health_2, amour_mod_2, weapon_2):
            text = 'Health: ' + str(health_2)
            message_display_left(text,50,25,15,BLACK)
            text = 'Armour: '+str(amour_mod_2[0][0]+amour_mod_2[1][0]+amour_mod_2[2][0]+amour_mod_2[3][0])
            message_display_left(text,50,40,15,BLACK)
            text = '    head: '+str(amour_mod_2[0][0])
            message_display_left(text,50,55,15,BLACK)
            text = '    chest: '+str(amour_mod_2[1][0])
            message_display_left(text,50,70,15,BLACK)
            text = '    arms: '+str(amour_mod_2[2][0])
            message_display_left(text,50,85,15,BLACK)
            text = '    legs: '+str(amour_mod_2[3][0])
            message_display_left(text,50,100,15,BLACK)
            text = 'Weapons: '
            message_display_left(text,50,115,15,BLACK)
            text = '    name: '+str(weapon_2[0])
            message_display_left(text,50,130,15,BLACK)
            text = '    attack: '+str(weapon_2[1])
            message_display_left(text,50,145,15,BLACK)
            pygame.display.flip()
        ...
        def player_1_turn(blank):
            ...
            turnstart = False
            wn.fill(BGCOLOR)
            player_1_stats(health_1,amour_mod_1,weapon_1)
            print(deck_1)
            print(hand_1)
            showcardsheld(hand_1)
            pygame.display.flip()
            #hold screen at menu until turn finnished
            global turnstart
            turnstart = True
            while turnstart:
                for event in pygame.event.get():
                    txt_button("End turn", 300, 250, 250, 50, WHITE, GREY, BLACK, player_1_turn,"")
                    txt_button("Close",300,150,200,50,GREEN,DARKGREEN,BLACK,terminate, "") #make close button
                    pygame.display.flip()
...
def displayCard(card,cardKey, deck):
    ...
    img_button(cardOneTemplate[0], cardOneTemplate[1], cardOneTemplate[2], cardOneTemplate[3], cardSelect, card, deck)
        #Description Layout
    ...
    for i in range(0,(len(cardAttributes))):
        message_display(str(cards[cardKey][i]), cardOneTemplate[0]+ 150, cardAttributeY,20, GREEN)
        cardAttributeY += 20
...
def cardPicker(array):
    ...
    while cardPicker == True:
        for event in pygame.event.get():
            ...
            if array[0] == "single":
                if array[1] == "palladin":
                    displayCard("1.jpg","0001", mainDeckSingle)
                    pygame.display.flip()
                elif array[1] == "barbarian":
                    displayCard("2.jpg","0002", mainDeckSingle)
                    pygame.display.flip()
                elif array[1] == "smith":
                    displayCard("3.jpg", "0002", mainDeckSingle)
                    pygame.display.flip()
            elif array[0] == "multi":
                global class1
                if array[1] == "palladin":
...
def cardSelect(cardFile, deck):
    if checkDeck(deck) == True:
        deck.append(cardFile)
    else:
